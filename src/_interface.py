...
class Simulator:
    def __init__(self, number_of_sites=0, populations_number=1, number_of_susceptible_groups=1, seed=None, \
                 sampling_probability=False, memory_optimization=False, genome_length=int(1e6),
        ...
        self.simulation = BirthDeathModel(number_of_sites=number_of_sites, populations_number=populations_number, \
                                          number_of_susceptible_groups=number_of_susceptible_groups, seed=seed,
                                          sampling_probability=sampling_probability, \
                                          memory_optimization=memory_optimization, genome_length=genome_length,
    ...
    def plot_infectious(self, population, haplotype, step_num, label_infectious, label_samples):
        infections, sample, time_points, lockdowns = self.simulation.get_data_infectious(population, haplotype,
                                                                                         step_num)
        if label_infectious == None:
            self.ax_2.plot(time_points, infections,
                           label='Infectious pop:' + str(population) + ' hap:' + self.simulation.calculate_string(
                               haplotype))
        ...
        if label_samples == None:
            self.ax.plot(time_points, sample, "--",
                         label='Samples pop:' + str(population) + ' hap:' + self.simulation.calculate_string(haplotype))
    ...
    def add_plot_susceptible(self, population, susceptibility_type, step_num=100, label_susceptible=None):
        ...
        susceptible, time_points, lockdowns = self.simulation.get_data_susceptible(population, susceptibility_type,
                                                                                   step_num)
        if label_susceptible == None:
            self.ax_2.plot(time_points, susceptible,
                           label='Susceptible pop:' + str(population) + ' sus:' + str(susceptibility_type))
