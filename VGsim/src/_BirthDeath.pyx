...
cdef class BirthDeathModel:
    cdef:
        ...
        Py_ssize_t user_seed, sites, hapNum, currentHapNum, maxHapNum, addMemoryNum, popNum, susNum, bCounter, dCounter, sCounter, mCounter, iCounter, swapLockdown, migPlus, migNonPlus, globalInfectious, countsPerStep, good_attempt, cCounter, conditionNum
        ...
        npy_int64[:,::1] susceptible, initial_susceptible,  infectious, 
        npy_int64[:,:,::1] infectByCondition, initial_infectious
        ...
        double[::1] conditionPopRate, cumuConditionRate
        double[:,::1] susceptibility, suscepTransition, immuneSourcePopRate, hapPopRate, migrationRates, effectiveMigration
        double[:,::1]  conditionRate,massivForThree, conditionPopHapRate, bRate, dRate, sRate, tmRate
        double[:,:,::1] hapMutType,  susceptHapPopRate, tEventHapPopRate, mRate
    ...
    def __init__(self, number_of_sites, populations_number, number_of_susceptible_groups, seed, sampling_probability, memory_optimization, conditionNum):
        self.user_seed = seed
        ...
        self.tmRate = np.zeros((self.maxHapNum, self.conditionNum), dtype=float)
        self.massivForThree = np.zeros((self.popNum, 3), dtype=float)
        self.conditionPopRate = np.zeros(self.popNum, dtype=float)
        self.tEventHapPopRate = np.zeros((self.popNum, self.maxHapNum, self.conditionNum), dtype=float)
        self.hapPopRate = np.zeros((self.popNum, self.maxHapNum), dtype=float)
        self.eventHapPopRate = np.zeros((self.popNum, self.maxHapNum, self.conditionNum, 4), dtype=float)
        ...
        self.conditionPopHapConRate = np.zeros((self.popNum, self.maxHapNum, self.conditionNum),dtype=float)
        self.infectByCondition = np.zeros((self.popNum, self.maxHapNum, self.conditionNum),dtype=np.int64)
        self.tEventConHapPopRate = np.zeros((self.popNum, self.maxHapNum, self.conditionNum), dtype=float)
        ...
        for hn in range(self.hapNum):
            for cn in range(self.conditionNum):
                self.bRate[hn, cn] = 2.0
                self.dRate[hn, cn] = 1.0
                self.sRate[hn, cn] = 0.01
                for s in range(self.sites):
                    self.mRate[hn, cn, s] = 0.01
            self.susceptibility[hn, 0] = 1.0

        for cn1 in range(self.conditionNum):
            for cn2 in range(self.conditionNum):
                if cn1 != cn2:
                    self.conditionRate[cn1, cn2] = 2
                else:
                    self.conditionRate[cn1, cn2] = 0
        
        for cn1 in range(self.conditionNum):
            self.cumuConditionRate[cn1] = 0
            for cn2 in range(self.conditionNum):
                self.cumuConditionRate[cn1] += self.conditionRate[cn1, cn2]
        ...
        self.initial_infectious = np.zeros((self.popNum, self.hapNum, self.conditionNum), dtype=np.int64)
        ...
        if self.globalInfectious == 0:
            for sn in range(self.susNum):
                if self.susceptible[0, sn] != 0:
                    ...
                    self.NewInfections(0, sn, 0, 0)
                    return

    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline void NewInfections(self, Py_ssize_t pi, Py_ssize_t si, Py_ssize_t hi, Py_ssize_t ci, Py_ssize_t num=1):
        self.susceptible[pi, si] -= num
        self.totalSusceptible[pi] -= num
        self.infectious[pi, hi] += num
        self.infectByCondition[pi, hi, ci] += num
        self.totalInfectious[pi] += num
        self.globalInfectious += num

    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline void NewRecoveries(self, Py_ssize_t pi, Py_ssize_t si, Py_ssize_t hi, Py_ssize_t ci,  Py_ssize_t num=1):
        self.susceptible[pi, si] += num
        self.totalSusceptible[pi] += num
        self.infectious[pi, hi] -= num
        self.infectByCondition[pi, hi, ci] -= num
    ...
    cdef void AddMemory(self):
        ...
        self.conditionPopHapRate = np.concatenate((self.conditionPopRate, np.zeros((self.popNum, self.addMemoryNum), dtype=float)), axis=1)
        self.conditionPopHapConRate = np.concatenate((self.conditionPopRate, np.zeros((self.popNum, self.addMemoryNum, self.conditionNum), dtype=float)), axis=1)
        self.infectByCondition = np.concatenate((self.infectByCondition, np.zeros((self.popNum, self.addMemoryNum, self.conditionNum),dtype=np.int64)), axis=1)
        self.tEventConHapPopRate = np.concatenate((self.tEventConHapPopRate, np.zeros((self.popNum, self.addMemoryNum, self.conditionNum), dtype=float)),axis=1)
    ...
    cdef void UpdateAllRates(self):
        ...
        for pn in range(self.popNum):
            for hn in range(self.currentHapNum): # hn - program number
                ...
                for cn in range(self.conditionNum):
                    ...
                    self.eventHapPopRate[pn, hn, cn, 3] = self.tmRate[hn, cn]
                    self.tEventHapPopRate[pn, hn, cn] = 0
                    for i in range(4):
                        self.tEventHapPopRate[pn, hn, cn] += self.eventHapPopRate[pn, hn, cn, i]
                    self.tEventConHapPopRate[pn, hn, cn] = self.tEventHapPopRate[pn, hn, cn] * self.infectByCondition[pn, hn, cn]
                    self.hapPopRate[pn, hn] += self.tEventConHapPopRate[pn, hn, cn]
            ...
            for hn in range(self.currentHapNum):
                for cn in range(self.conditionNum):
                   self.conditionPopHapConRate[pn, hn, cn] = self.cumuConditionRate[cn] * self.infectByCondition[pn, hn, cn]
                   self.conditionPopHapRate[pn, hn] += self.conditionPopHapConRate[pn, hn, cn] 
                self.conditionPopRate[pn] += self.conditionPopHapRate[pn, hn]
            

            self.popRate[pn] = self.infectPopRate[pn] + self.immunePopRate[pn] + self.conditionPopRate[pn]
            self.massivForThree[0] = self.infectPopRate[pn]
            self.massivForThree[1] = self.immunePopRate[pn]
            self.conditionPopRate[2] = self.conditionPopRate[pn]
        ...
        for hn in range(self.currentHapNum):
            for cn in range(self.conditionNum):
                for sn in range(self.susNum):
                    if self.bRate[self.numToHap[hn], cn]*self.susceptibility[self.numToHap[hn], sn] > maxEffectiveBirth:
                        maxEffectiveBirth = self.bRate[self.numToHap[hn], cn]*self.susceptibility[self.numToHap[hn], sn]
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef inline double BirthRate(self, Py_ssize_t pi, Py_ssize_t hi, Py_ssize_t ci): # hi - program number
        cdef double ps = 0.0 
        ...
        for cn in range(self.conditionNum):
            self.bRate[self.numToHap[hi], cn] = self.bRate[self.numToHap[hi], cn] * ps
        cdef double hi_ps = 0.0
        for cn in range(self.conditionNum):
            hi_ps += self.bRate[self.numToHap[hi], cn]
        return hi_ps
    ...
    cdef void PrepareParameters(self, Py_ssize_t iterations):
        ...
        if self.first_simulation == False:
            ...
            for pn in range(self.popNum):
                ...
                for hn in range(self.hapNum):
                    for cn in range(self.conditionNum):
                        self.initial_infectious[pn, hn, cn] = self.infectByCondition[pn, hn, cn]
    ...
    cdef Py_ssize_t GenerateEvent(self):
        ...
        if self.totalRate > choose:
            ...
            ei, self.rn = fastChoose1(self.massivForThree[pi], self.totalRate, self.rn)
            if ei == 1:
                ...
                ci, self.rn = fastChoose1(self.tEventConHapPopRate[pi, hi], self.hapPopRate[pi, hi], self.rn)
                ei, self.rn = fastChoose1(self.tEventHapPopRate[pi, hi], self.tEvenContHapPopRate[pi, hi, ci], self.rn)
                if ei == BIRTH:
                    self.Birth(pi, hi, ci)
                elif ei == DEATH:
                    self.Death(pi, hi, ci)
    ...
    cdef void UpdateRates(self, Py_ssize_t pi, bint infect, bint immune, bint migration):
        ...
        if infect:
            ...
            for hn in range(self.currentHapNum):
                for cn in range(self.conditionNum):
                    self.eventHapPopRate[pi, hn, cn, 0] = self.BirthRate(pi, hn, cn)
                    tmp = (self.eventHapPopRate[pi, hn, cn, 0] +
                           self.eventHapPopRate[pi, hn, cn, 1] +
                          self.eventHapPopRate[pi, hn, cn, 2] +
                          self.eventHapPopRate[pi, hn, cn, 3] )
                    self.tEventHapPopRate[pi, hn, cn] = tmp
                    self.tEventConHapPopRate[pi, hn, cn] = self.tEventHapPopRate[pi, hn, cn] * self.infectByCondition[pi, hn, cn]
                    self.hapPopRate[pi, hn] += self.tEventConHapPopRate[pi, hn, cn]
        ...
        if infect or immune:
            self.popRate[pi] = 0
            self.popRate[pi] = self.infectPopRate[pi] + self.immunePopRate[pi]+ self.conditionPopRate[pi]
            self.massivForThree[0] = self.infectPopRate[pi]
            self.massivForThree[1] = self.immunePopRate[pi]
            self.massivForThree[2] = self.conditionPopRate[pi]
            self.totalRate = 0.0
            for pn in range(self.popNum):
                self.totalRate += self.popRate[pn]
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void ConditionTransition(self, Py_ssize_t pi):
        cdef:
        ...
        tsi, self.rn = fastChoose1_skip(self.conditionRate[ssi], self.cumuConditionRate[ssi], self.rn, ssi)

        self.infectByCondition[pi, hsi, ssi] -= 1
        self.infectByCondition[pi, hsi, tsi] += 1
        self.conditionPopHapConRate[pi, hsi, ssi] = self.cumuConditionRate[ssi] * self.infectByCondition[pi, hsi, ssi]
        self.conditionPopHapConRate[pi, hsi, tsi] = self.cumuConditionRate[tsi] * self.infectByCondition[pi, hsi, tsi]
        self.UpdateRates(pi, False, True, False)

        self.cCounter += 1
        self.events.AddEvent(self.currentTime, CONDITION, ssi, pi, tsi, 0) #ssi, tsi -- condition, add haplotype hsi??

    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void Birth(self, Py_ssize_t pi, Py_ssize_t hi, Py_ssize_t ci): # hi - program number
        cdef double ws = 0.0
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void Death(self, Py_ssize_t pi, Py_ssize_t hi, Py_ssize_t ci, bint add_event = True): # hi - program number
        self.NewRecoveries(pi, self.suscType[self.numToHap[hi]], hi, ci)
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void Sampling(self, Py_ssize_t pi, Py_ssize_t hi, Py_ssize_t ci): # hi - program number
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cdef void Mutation(self, Py_ssize_t pi, Py_ssize_t hi, Py_ssize_t ci): # hi - program number
        cdef:
            bint check = True
            Py_ssize_t ohi, mi, digit4, AS, DS, nhi

        ohi = self.numToHap[hi] # ohi - haplotype
        mi, self.rn = fastChoose1(self.mRate[ohi, ci], self.tmRate[hi, ci], self.rn)
    ...
    cdef Py_ssize_t GenerateMigration(self):
        ...
        ci, self.rn = fastChoose1(self.conditionRate[spi], self.cumuConditionRate[spi], self.rn)
        si, self.rn = fastChoose2(self.susceptible[tpi], self.totalSusceptible[tpi], self.rn)

        p_accept = self.effectiveMigration[spi, tpi]*self.bRate[self.numToHap[hi], ci]*self.susceptibility[self.numToHap[hi], si]/self.maxEffectiveBirthMigration[tpi]
        if self.rn < p_accept:
            self.NewInfections(tpi, si, hi, ci)
    ...
    cdef void Restart(self):
        ...
        for pn in range(self.popNum):
            ...
            for hn in range(self.hapNum):
                for cn in range(self.conditionNum):
                    self.infectious[pn, hn] += self.initial_infectious[pn, hn, cn]
                    self.totalInfectious[pn] += self.initial_infectious[pn, hn, cn]
                    self.globalInfectious += self.initial_infectious[pn, hn, cn]
        for pn in range(self.popNum):
            self.CheckLockdown(pn)
        self.UpdateAllRates()
    ...
    def check_haplotype(self, haplotype):
        ...
        elif isinstance(haplotype, str):
            if haplotype.count("A") + haplotype.count("T") + haplotype.count("C") + haplotype.count("G") + haplotype.count("*") != self.sites:
                raise ValueError('Incorrect haplotype. Haplotype should contain only \"A\", \"T\", \"C\", \"G\", \"*\" and lenght of haplotype should be equal number of mutations sites.') 
    ...
    def set_transmission_rate(self, rate, haplotype, condition):
        ...
        self.check_index(condition, self.conditionNum, 'number of states')
        haplotypes = self.create_list_for_cycles(haplotype, self.hapNum)
        conditions = self.create_list_for_cycles(condition, self.conditionNum)
    ...
    def set_recovery_rate(self, rate, haplotype, condition):
        ...
        for hn in haplotypes:
            for cn in conditions:
                self.dRate[hn, cn] = rate

    def set_sampling_rate(self, rate, haplotype, condition):
        self.check_index(haplotype, self.hapNum, 'haplotype', True)
        self.check_value(condition, self.conditionNum, 'number of states')
        conditions = self.create_list_for_cycles(condition, self.conditionNum)
        ...
        if self.sampling_probability == True:
            ...
            for hn in haplotypes:
                for cn in conditions:
                    deathRate = self.dRate[hn, cn] + self.sRate[hn, cn]
                    self.dRate[hn, cn] = (1 - rate) * deathRate
                    self.sRate[hn, cn] = rate * deathRate
            ...
            for hn in haplotypes:
                for cn in conditions:
                    self.sRate[hn, cn] = rate

    def set_mutation_rate(self, rate, haplotype, mutation, condition):
        ...
        for hn in haplotypes:
            for cn in conditions:
                for s in sites:
                    self.mRate[hn, cn, s] = rate  

    @property
    def condition_rate(self):
        return self.conditionRate
    ...
    def set_susceptibility(self, rate, haplotype, susceptibility_type):
        ...
        if isinstance(haplotype, str):
            haplotypes = self.create_list_haplotypes(haplotype)
            if isinstance(susceptibility_type, int):
                for hn in haplotypes:
            ...
            if isinstance(susceptibility_type, int):
                self.susceptibility[haplotype, susceptibility_type]
            else:
                for sn in range(self.susNum):
    ...
    def set_susceptible(self, amount, source_type, target_type, population):
        ...
        if isinstance(population, int):
            self.check_amount_sus(amount, source_type, target_type, population)

            self.initial_susceptible[population, source_type] -= amount
            self.initial_susceptible[population, target_type] += amount
        elif population == None:
            for pn in range(self.popNum):
    ...
    def set_infectious(self, amount, source_type, target_haplotype, population, condition):
        if self.first_simulation:
            raise ValueError('This function is available only before first simulation!')
        self.check_amount(amount)
        self.check_index(source_type, self.susNum, 'susceptibility type')
        self.check_haplotype(target_haplotype)
        self.check_index(population, self.popNum, 'population')

        if isinstance(population, int):
            self.check_amount_inf(amount, source_type, target_haplotype, population)

            self.initial_susceptible[population, source_type] -= amount
            
            self.initial_infectious[population, target_haplotype, condition] += amount
        elif population == None:
            for pn in range(self.popNum):
                self.check_amount_inf(amount, source_type, target_haplotype, pn)

                self.initial_susceptible[pn, source_type] -= amount
                self.initial_infectious[pn, target_haplotype, condition] += amount
    ...
    def set_migration_probability(self, probability, total_probability, source, target):
        ...
        if isinstance(probability, float):
            if isinstance(source, int):
                if isinstance(target, int):
                    if source == target:
                        raise IndexError('Source and target population shouldn\'t be equal!')
    ...
    cdef void Propensities(self):
                ...
                for sn in range(self.susNum):
                    for hn in range(self.hapNum):
                        for cn in range(self.conditionNum):
                            ...
                            self.infectious[spn, hn]*self.bRate[hn, cn]*self.susceptibility[hn, sn]*self.migrationRates[spn, spn]
        ...
        for pn in range(self.popNum):
            ...
            for hn in range(self.hapNum):
                #Recovery
                for cn in range(self.conditionNum):
                    self.PropensitiesRecovery[pn, hn] = self.dRate[hn, cn]*self.infectious[pn, hn]
                ...
                #Sampling
                for cn in range(self.conditionNum):
                    self.PropensitiesSampling[pn, hn] = self.sRate[hn, cn]*self.infectious[pn, hn]*self.samplingMultiplier[pn]
                ...
                for s in range(self.sites):
                    for cn in range(self.conditionNum):
                        for i in range(3):
                            self.PropensitiesMutatations[pn, hn, s, i] = self.mRate[hn, cn, s]*self.hapMutType[hn, s, i]/\
            ...
            for hn in range(self.hapNum):
                for cn in range(self.conditionNum):
                    for sn in range(self.susNum):
                        ...
                        for spn in range(self.popNum):
                            self.PropensitiesTransmission[tpn, hn, sn] += self.bRate[hn, cn]*self.susceptibility[hn, sn]*self.migrationRates[tpn, spn]*\
